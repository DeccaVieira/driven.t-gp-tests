...
describe('POST /booking', () => {
  ...
  describe('When token is valid', () => {
    it('Should response with 404 if RoomId is not exists', async () => {
      const user = await createUser();
      const hotel = await createHotel();
      const token = await generateValidToken(user);
      const createRoom = await createRoomWithHotelId(hotel.id);
...
describe('PUT /booking/:bookingId', () => {
  ...
  describe('when token is valid', () => {
    ...
    it('should response with status 404 if roomId not exist', async () => {
      const user = await createUser();
      const token = await generateValidToken(user);
      const enrollment = await createEnrollmentWithAddress(user);
      const ticketType = await createTicketTypeWithHotel();
...
//     const payment = await createPayment(ticket.id, ticketType.price);
//     const createdHotel = await createHotel();
//     const updateHotel = await createHotel();
//     const createdRoom = await createRoomWithHotelId(createdHotel.id);
//     const updateRoom = await createRoomWithHotelId(updateHotel.id);
//     const booking = await createBookingFaker(user.id, createdRoom.id);
